...
aws ec2 create-tags --resources "$VPC_ID" --tags Key=Name,Value=VPC-PREPROD-cnlm-cam
...
aws ec2 create-tags --resources "$SUBNET_PUB_A" --tags Key=Name,Value=subnet-pub-cnlm-cam-A
...
SUBNET_PUB_B=$(aws ec2 create-subnet \
    ...
    --query 'Subnet.SubnetId' --output text)
aws ec2 create-tags --resources "$SUBNET_PUB_B" --tags Key=Name,Value=subnet-pub-cnlm-cam-B
...
SUBNET_PRIV_A=$(aws ec2 create-subnet \
    ...
    --availability-zone "${REGION}a" \
    --query 'Subnet.SubnetId' --output text)
aws ec2 create-tags --resources "$SUBNET_PRIV_A" --tags Key=Name,Value=subnet-priv-cnlm-cam-A
...
SUBNET_PRIV_B=$(aws ec2 create-subnet \
    ...
    --availability-zone "${REGION}b" \
    --query 'Subnet.SubnetId' --output text)
aws ec2 create-tags --resources "$SUBNET_PRIV_B" --tags Key=Name,Value=subnet-priv-cnlm-cam-B
echo "    Sous-réseaux : $SUBNET_PUB_A | $SUBNET_PUB_B | $SUBNET_PRIV_A | $SUBNET_PRIV_B"

echo "=== Création Internet Gateway ==="

IGW_ID=$(aws ec2 create-internet-gateway --query 'InternetGateway.InternetGatewayId' --output text)
aws ec2 create-tags --resources "$IGW_ID" --tags Key=Name,Value=IGW-PREPROD-cnlm-cam
...
aws ec2 create-tags --resources "$NAT_GW_ID" --tags Key=Name,Value=NAT-PREPROD-cnlm-cam
...
aws ec2 create-tags --resources "$RT_PUB_ID" --tags Key=Name,Value=RT-Public-PREPROD-cam
...
aws ec2 create-tags --resources "$RT_PRIV_ID" --tags Key=Name,Value=RT-Private-PREPROD-cam
...
SG_EC2_ID=$(aws ec2 create-security-group \
    --group-name SG-EC2-PREPROD-cnlm-cam \
